 ...
 *)
Print string. 
(*This is just the definition of a string*)
 ...
 *)
Check ("f"%char : char).
Check ("string example" : String.string).
  ...
  Entry None
    (Cons Empty "t"%char
      (Entry None
        (Cons Empty "h"%char
          (Entry None
            (Cons (Cons Empty "a"%char (Entry None (Cons Empty "t"%char (Entry (Some "1 person or thing indicated, named, or understood") Empty)))) "e"%char
              (Entry (Some "denoting person(s) or thing(s) already mentioned")
                (Cons Empty "n"%char
                  (Entry (Some "next; after that.") Empty)))))))).
...
Fixpoint find word dictionary : option string :=
  ...
  | Entry definition listDictionary =>
    ...
    | String char restWord => find_list char restWord listDictionary
    end
  end
with find_list char word listDictionary : option string :=
  match listDictionary with
  | Empty => None
  | Cons restListDictionary label restDictionary =>
    if char =c label then
      find word restDictionary
    else
      find_list char word restListDictionary
  end.
...
Fixpoint new word def :=
  ...
  | String char restWord => Entry None (Cons Empty char (new restWord def))
  end.

(*Debug purposes.*)
Eval compute in find "that" (new "that" "Some 1 person or thing indicated, named, or understood").
